...
export function generateOverlayHTML(place: Place): string {
  ...
  // Note: 카카오맵 오버레이는 HTML 문자열로 렌더링되므로 Next.js Image 컴포넌트를 사용할 수 없습니다.
  // 대신 네이티브 img 태그를 사용하되, loading="lazy"를 추가하여 성능을 최적화합니다.
  const imageHtml = locationImage
    ? `<img src="${locationImage}" alt="${name}" loading="lazy" style="width: 100%; height: 100%; object-fit: cover;" />`
