...
export function PlaceCard({
  ...
  return (
    <div
      ...
      onClick={onClick}
      className={[
        'p-[var(--spacing-4)] cursor-pointer transition-all hover:bg-[var(--color-background-tertiary)]',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
    >
      <div className="flex gap-[var(--spacing-4)]">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-heading-5 text-[var(--color-text-primary)] mb-[var(--spacing-1)] truncate">
            {name}
          </h3>
          <PlaceAddress address={address} />
          <div className="bg-[var(--color-brand-primary)]/10 rounded-lg p-[var(--spacing-2)] mb-[var(--spacing-2)]">
            <div className="flex items-center gap-[var(--spacing-1)] mb-[var(--spacing-1)]">
              <Camera className="w-3 h-3 text-[var(--color-brand-secondary)]" />
              <span className="text-caption-bold text-[var(--color-brand-secondary)]">
                {PLACE_LABELS.SCENE}
              </span>
            </div>
            <p className="text-caption text-[var(--color-brand-secondary)] line-clamp-2">
              {description}
            </p>
          </div>
